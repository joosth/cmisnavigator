...
class Repository(object):
    ...
    def createDocumentFromSource(self,
        ...
        postUrl = parentFolder.getChildrenLink()
        sourceNode = self.getObject(sourceId)
        
        
        # Remove the object Id; if it would be left in here it would just create a multifile relationship
        
        idprops=sourceNode.xmlDoc.getElementsByTagName("cmis:propertyId")
        for idprop in idprops:
            print idprop.getAttribute("propertyDefinitionId")
            if idprop.getAttribute("propertyDefinitionId")=="cmis:objectId":
                sourceNode.xmlDoc.getElementsByTagName("cmis:properties")[0].removeChild(idprop)                   
        xmlEntry=sourceNode.xmlDoc.toxml(encoding='utf-8')
        
        print xmlEntry
        
        result = self._cmisClient.post(postUrl.encode('utf-8'), xmlEntry, ATOM_XML_ENTRY_TYPE)
        #print result
        #newDoc= Document(self._cmisClient, self, xmlDoc=result)
        
        # TODO read file and upload again ...
        ...
        #f = open("/home/joost/wf4p-header.txt", 'rb')
